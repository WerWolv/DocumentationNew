...
description: "Documentation for Importing Modules."
published: true
toc:
  visible: true
---
...
A [preprocessor directive](/pattern-language/core-language/preprocessor). The preprocessor replaces this directive with all the lexical tokens from the included file _including_ all the preprocessor defines. On inclusion, this system maintains its own list of files marked with [`#pragma once`](/pattern-language/core-language/importing-modules#include-guards).
...
The `import` statement can accept a string argument instead. This behaves the same as [`#include`](/pattern-language/core-language/importing-modules#include-directive). This can be used when your module's name or folder contains a keyword, since keywords can't be used as identifiers.
