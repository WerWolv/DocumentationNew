---
title: "Preprocessor"
description: "Documentation for Preprocessor."
published: true
toc:
  visible: true
---
...
This directive allows inclusion of other files into the current program. The content of the specified file gets copied directly into the current file. See [importing modules](/pattern-language/core-language/importing-modules#include-directive) for more info.
...
The `import` statement and `#include` directive each keep a separate list of files marked with `#pragma once`. That means that a set of headers using one system for importing should stick to it. See [Importing Modules](/pattern-language/core-language/importing-modules) for more info.
...
This pragma specifies the pattern author shown in the [Content Store](/imhex/misc/content-store). If you need to specify multiple authors, use only one `#pragma author`.

#### `description`

**Possible values:** Any string value **Default:** `Unspecified`

This pragma specifies the description shown for the pattern in the [Content Store](/imhex/misc/content-store).
